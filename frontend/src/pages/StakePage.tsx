...
export default function StakePage({ account }: Props) {
  ...
  const onWithdraw = async () => {
    ...
    setBusy(true);
    try {
      const wei = BigInt(Math.floor(parseFloat(withdrawEth) * 1e18));
  ...
  return (
    <div className="space-y-6 max-w-md">
      ...
      <div className="space-y-3 rounded-lg border bg-white p-4">
        <div className="text-sm font-medium">Stake ETH</div>
        <input
          type="number"
          min="0"
          step="0.0001"
      ...
      <div className="space-y-3 rounded-lg border bg-white p-4">
        <div className="text-sm font-medium">Unstake ETH</div>
        <input
          type="number"
          min="0"
          step="0.0001"
